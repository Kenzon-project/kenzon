...
class CarrinhoView(ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not product.quantidade_estoque:
            return Response({"message": "Produto sem estoque."}, status=status.HTTP_400_BAD_REQUEST)
...
class CarrinhoEdit(RetrieveUpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        carrinho_produto = self.get_queryset().filter(produto_id=self.kwargs['product_id'], carrinho=self.request.user.carrinho).first()
